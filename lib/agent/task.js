...
function Task(doc, req, res) {
  ...
  onHeaders(res, function(){
    ...
    if (_.has(self.doc.manual, 'type')) {
      var ct = content_type.parse(this.getHeader('content-type'));

      ct.parameters['type'] = self.doc.manual.type;
      this.setHeader('content-type', content_type.format(ct));
    }
    ...
    if (self.etag.code) {
      s$cache  = http.request({
                      ...
                      , hostname  : 'localhost'
                      , port      : 8787
                      , headers   : _.defaults({ 'x-tm-status-code' : res.statusCode }, headers)
