...
function Task(doc, req, res) {
  ...
  // todo [akamel] this won't really work since express will always set a content-type
  // ---- we will present in the worker
  if (this.doc.manual.output) {
    if (_.has(this.doc.manual.output, 'content-type')) {
      res.set('Content-Type', this.doc.manual.output['content-type']);
    }
  }
