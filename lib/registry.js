...
AgentRegistry.prototype.findOne = function(task) {
  ...
  if (_.size(options)) {
    var weights = _.map(options, function(i){
      var len     = _.size(i.info.workers)
        , usage   = len / i.info.capacity
        ;

      // todo [akamel] for each task we allocate, assign count directly / don't wait for confirmation on the next ping // reduce rebalancing lag
