...
// todo [akamel] this method no longer works.
Dispatcher.prototype.next = function(err, item) {
  ...
  if (err) {
    ...
    if (item.res) {
      item.res.status(500).send(err_obj);
    }
    ...
    // todo [akamel] temp delay end until the client has had time to join/subscribe
    // _.delay(function(){
    this.relay.getSocket().in(id).emit('script-stdio', {
        type      : 'stdout'
      , text      : null
      , execution : { id : id }
    });
...
Dispatcher.prototype.push = function(doc, req, res /* res | function */) {
  ...
  p$req = !_.isUndefined(res) || req.isSocket
            ? Promise.cast(req)
